...
def write_skill(
    ...
    if has_drills:
        body += (
            ...
            "Qualification is answer-key blind but skillcard-present: `prepare` packages "
            "the Drill's exact linked Pattern/AP cards, and only the Success Check and "
            "Common Failures remain hidden. A valid qualification is binary PASS or FAIL; "
            "a setup, tool, or package failure is INVALID and says nothing about craft. "
            "Linked practice may teach through Drill Instructions, packaged Pattern/AP "
            "cards, external material, or a declared combination. "
            "Use repeated `--drill` arguments to chain compatible Drills from one "
            "domain. Never expose `controller/` to the taker.\n\n"
            "The current run purpose is `skillset-improvement`: use the evidence to "
            "qualify cards and Skillset Memory, not to claim persistent model-memory "
            "or weight changes. Record a valid application miss as a transferable "
            "mistake/correction/prevention lesson, never as a named model's weakness. "
            "Drill guidance should decay as capability stabilizes; ordinary work then "
            "leans on Patterns and Action Protocols.\n\n"
    ...
    if has_code_study:
        body += (
            ...
            "PASS-guided alternative, property-sensitive checks, and an explicit "
            "comparison. Use `prepare` → `freeze-discovery` → `open-guidance` → "
            "`freeze-work` → `reveal` → `accept-audit` → `finalize`. The structured "
            "audit binds deciding facts to frozen source lines before craft grading. "
            "A valid card qualification is "
