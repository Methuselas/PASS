...
- Project source points along a consistent construction direction for that view, then mark where each projection meets the target section/surface.
...
- Assume a straight projection produces an exact non-stretch material wrap around every curved surface; a label-on-cylinder projection is an approximation.
