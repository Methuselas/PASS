...
- Around the shoulder and elbow, continue using functional groups rather than starting over with an inventory of named muscles. Deduce each group's action from its position relative to the joint axes.
...
- Separate the carpal/digital extensor and flexor masses by the axis they cross. The extensor group runs on the external/front side of the mechanism while the flexor group occupies the opposing internal/back route and carries greater mass.
...
- Do not make flexors and extensors equal in volume when the studied carnivore shows a stronger flexion group.
...
- Do not infer exact muscle size or action from simplified functional plates when the intended species or pose differs; use the live/reference anatomy to set the final amount.
