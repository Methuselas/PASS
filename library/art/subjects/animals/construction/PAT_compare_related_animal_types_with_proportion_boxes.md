...
- Compare related examples side by side: contrast draft and saddle horses, then use the same boxed thinking for a colt and for zebra-versus-horse proportion.
- Let age or type change the whole proportional ensemble rather than resizing one isolated feature.
- Treat named type traits as observations to test against the actual subject or reference, not as fixed biological constants.
