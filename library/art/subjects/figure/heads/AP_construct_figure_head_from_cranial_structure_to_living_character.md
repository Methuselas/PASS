...
3. **Use the lightest structural variant that solves the actual problem.** When the turn or tilt drifts, use an applicable perspective-cage or rotation/plane variant. When feature placement drifts on a sound head block, use the divided-cranial-ball feature-placement variant. When the head reads too round or mask-like, use an applicable planar/skull-boundary variant. These are conditional tools under the same head-construction decision, not a checklist to load all at once.
...
7. **Treat hair as a head mass before strand information.** When hair materially changes silhouette or character, use the large-mass-before-strands variant. Keep the hairstyle attached to the skull and subordinate individual strand information to the accepted head volume and stage ceiling.
