...
Mentally executing code this way is called tracing, or cognitive compiling. Tools can generate the same picture automatically — Philip Guo's Python Tutor steps through execution and renders the frames, making visible, for instance, when two variables refer to the same list, so a change made through one appears through the other. Hermans's point in favour of doing it by hand anyway is that the manual pass forces the detailed examination that produces the understanding; the tool shows you a result you did not compute.
