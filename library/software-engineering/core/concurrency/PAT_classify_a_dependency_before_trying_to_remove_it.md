...
- Look for a schedule before calling a loop nest with a true dependency sequential. Give each iteration a step later than every iteration it reads from; iterations on the same step read nothing from one another and can run together. Where each cell of a table reads its left, upper and upper-left neighbours, the steps are the anti-diagonals. On a 12,000-by-12,000 edit-distance table with 24 threads, the row-by-row sweep took 302 ms, and running whole anti-diagonals took 116 ms. Cutting the table into 64-by-64 tiles and running each tile-diagonal's tiles together took 25 ms. Tiles of 1,024 by 1,024, at most 12 to a diagonal, took 70 ms.
...
- Don't remove a true dependency by ignoring it. Violating a real ordering does not produce a slower program or a rare failure — it produces a wrong answer, sometimes reproducibly enough to be mistaken for correct on small inputs. Splitting the rows of that edit-distance table among threads returned a distance of 255 where the answer was 6,239.
...
- If a true dependency survives, is there a schedule under which each step's iterations read nothing from one another?
- If none exists, what parallelizes around it?
