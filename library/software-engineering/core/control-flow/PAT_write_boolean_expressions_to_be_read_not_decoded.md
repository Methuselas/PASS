...
- Parenthesize fully rather than relying on precedence — except inside a comparison chain in a language that gives chains their own meaning, where parentheses turn one range test into a boolean compared with a number. Extra parentheses cost nothing, and they remove a demand on a reader who may be moving between languages with different rules. When a chain of comparisons could group more than one way, the parentheses are also what tell the compiler which one you meant.
