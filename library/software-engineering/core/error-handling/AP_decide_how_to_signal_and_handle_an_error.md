...
6. **Never hide the error, and heed the warnings.** Reject default values, empty collections, silent returns, and swallowed exceptions; and treat any warning the change raises — from the compiler, linter or type checker — as a possible bug to fix or explicitly suppress with a reason. `PAT_dont_hide_errors` and `PAT_treat_compiler_warnings_as_potential_bugs` own the two halves of this.
