...
- Make a placeholder for code not yet written fail when it is called. A stub whose body does nothing returns an empty result no caller can tell from a real one — the same disguise as doing nothing — so have it raise a not-implemented error that names what is missing.
