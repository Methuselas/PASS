...
**THEN** define a small dedicated type (class or struct) for it rather than reaching for a general type like a list or a pair, so the type is self-describing and its shape is enforced — by the compiler where there is one, otherwise where the object is built and by a type checker.
...
- Get real type safety: a dedicated type fixes the field count and names, so a reader of `location.latitude` cannot mistake it for the longitude, and a wrong number of values is rejected where the object is built rather than where it is first indexed.
- Close the one swap a named type still allows: at construction, positional arguments of the same type can be passed in the wrong order and nothing objects. Where the language supports it, require the fields by name.
...
## Don't
- Don't represent a location as a list of doubles (or a list of lists of doubles); nothing explains the type, latitude and longitude are easily reversed, and a list with too few or too many values is accepted when built and fails only at the first index that reaches past it.
...
- Is a wrong shape rejected where the value is built — a wrong count by the type, swapped fields by named construction — or only where it is first used?
