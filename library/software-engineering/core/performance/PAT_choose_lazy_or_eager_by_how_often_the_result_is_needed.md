...
- Defer tidying to the operation that has to traverse the untidy part anyway. A structure whose insert simply appends to a list, leaving the list disorganised, and whose remove-the-smallest has to scan that list to find the new smallest, can reorganise the list during that same scan; the inserts stay constant time, and the scan's extra work is paid for by the shorter list it leaves behind for the next one. The bound this gives is amortized: after a long run of cheap operations the next scan is long, so the sequence is fast while that one call is not — acceptable for throughput, not for a latency budget.
