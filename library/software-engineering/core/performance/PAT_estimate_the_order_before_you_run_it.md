...
## Do
- Read the growth off the structure. A loop running once per item grows linearly. A loop nested inside another grows as the product of the two limits, which for two passes over the same collection is the square. Halving the candidates each time around is logarithmic. Splitting the input, solving the pieces and combining them has no single answer — that shape is read by the next item. Anything that enumerates permutations grows factorially — time it for five items and six will take six times as long, seven forty-two times as long.
- Weigh the combining against the pieces when a routine calls itself. Where the combining dominates, its cost is the answer; where the pieces dominate, it is their count compounded over the depth; where the two balance, it is the combining cost multiplied by the depth. Two half-sized pieces with a single merging pass balance, which is why that arrangement is linear times a logarithm — four half-sized pieces with the same merge is square, and the only thing that changed was how many pieces there were.
...
- Where a square-law step sits on the critical path, look for a restatement that splits the input and recombines the pieces — but check the count of pieces rather than assuming the split alone buys the improvement, because the saving comes from solving fewer or smaller copies and a split that solves just as many is the same cost rewritten.
...
- Don't picture the worst case as an exotic input you will probably never meet. For a great many operations it is the routine one: a lookup that finds nothing has examined every candidate before it can say so, and lookups that find nothing are common in any system that checks for absence — a cache miss, a permission that is not granted, a record that has not been created yet. Where the miss is the common path, the worst-case figure is not a pessimistic bound on your workload, it is a description of it.
- Don't read a shared worst-case classification as a promise that two routines behave alike on easy input. One of them may be able to finish early when the input is favourable while the other structurally cannot: a search that stops at the first match and a search that counts every match are both linear on the worst input, but only the first has a better best case, and on inputs where the answer sits near the front that difference is the entire performance story. The notation records the ceiling and says nothing about whether the floor is lower.
