...
- State the input model on what the method receives, not on the structures it builds from it. A search tree built from randomly ordered keys averaged a depth of 20 at a hundred thousand keys, where a uniformly random tree shape of the same size averaged 558; a model or benchmark that draws tree shapes at random predicts searches 28 times too slow for the tree the method actually builds.
