...
The same bound reaches past sorting. A priority queue that orders its elements only by comparing them cannot make both insertion and removal of the smallest faster than logarithmic, because inserting n elements and removing them all would then sort them in fewer than n log n comparisons. So any comparison-based queue has at least one logarithmic operation, and the way past it is the same: when the keys are integers from a known, bounded range, structures indexed by the key's bits answer insert, delete, member, successor and predecessor in time that grows with the logarithm of the number of bits rather than of the number of elements. They carry the counting sort's cost in a different form — space and creation time proportional to the size of the range unless only the non-empty parts are allocated — so they pay where the range is fixed and the operations are many, and not for a handful of operations on a sparse range.
