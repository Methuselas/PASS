...
- Find that size by measuring on the machine that will run the code, not by counting operations. Operation-count analyses of the seven-product matrix method placed its crossover at dimensions of eight to twelve; benchmarks on real systems, where caches and pipelines decide, placed it between roughly four hundred and two thousand, and on some systems the recursive method never overtook the simple one at all. Where the code will run on machines you do not control, set the threshold by measuring when it is installed or first run rather than hard-coding one number, and allow for the answer that the recursion should never be taken.
