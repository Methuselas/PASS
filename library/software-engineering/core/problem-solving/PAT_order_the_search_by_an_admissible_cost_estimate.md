...
## Notes
A heuristic of zero everywhere is always admissible and degrades this exactly to the shortest-path search it is built on top of, which is a useful sanity check on an implementation: disable the heuristic, and the algorithm should slow toward blind-search speed while still finding the same optimum. If it finds a different or worse answer instead, the bug is in the ordering machinery rather than in the heuristic.
