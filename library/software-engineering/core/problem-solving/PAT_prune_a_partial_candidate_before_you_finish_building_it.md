...
- Expand where the outcome is still uncertain, and treat which candidate to expand next as a free choice you are making rather than one the structure forces. The order in which pieces are decided changes how early contradictions surface. Branch on the decision with the fewest remaining options, and take any decision left with a single option as a forced move before branching at all. Each branching multiplies the search by its number of options. A satisfiability search that branched on a shortest open clause visited 212 nodes on average at 30 variables and 2,685 at 60; branching on the first open clause instead visited 6,862 and 6.5 million.
