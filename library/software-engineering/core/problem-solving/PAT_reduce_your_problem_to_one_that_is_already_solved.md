...
- Reduce to the least general target that still covers your problem, because the standard problems form a family ordered by generality and the cost runs the other way along it. Matching, maximum flow, minimum-cost flow and general linear programming form such a chain, each instance solvable by the target below it, and the specialised methods routinely beat the general solver by orders of magnitude on the problems they were built for. The most powerful target is simultaneously the one most likely to accept your translation and the one least likely to be worth using.
...
## Don't
- Don't translate into a harder problem than you need. Requiring that the answers come out as whole numbers reads like a detail and can move a problem from routinely solvable to intractable, so a constraint added for tidiness deserves the same scrutiny as one the domain demands. Know the exception before avoiding a target on this account: in a flow problem whose capacities are all whole numbers, the augmenting-path method returns a whole amount on every link, so a one-to-one pairing — where each candidate pair is either taken or not — reduces to flow with every capacity one and pays nothing for the requirement. Checked on 300 small pairing problems: the unit-capacity flow put 0 or 1 on every link and matched the largest pairing found by exhaustive search every time.
