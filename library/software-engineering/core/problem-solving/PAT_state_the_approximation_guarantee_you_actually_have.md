...
- Prove the ratio against a quantity you can compute and that the optimum can never beat, because the optimum is exactly what you cannot compute. Every cover of a network's links must include an end of each link in a set of links that share no ends, so that set's size bounds the smallest cover from below, and taking both ends of each of its links gives a cover at most twice as large. Every route through all the points is at least as long as the cheapest tree connecting them, and walking that tree gives a route at most twice as long wherever going direct is never longer than a detour. Dropping a whole-number requirement gives the same kind of bound from a relaxed problem. The bound computed on each instance also reports how far any answer can be from the best, including a heuristic's, but it reports a ceiling, not the gap: on 300 random networks a highest-degree heuristic was never more than twice the bound, while exhaustive search put its true worst at 1.40 times the optimum.
