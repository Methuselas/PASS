...
- Under the C++20 floor, use `contains` for a boolean membership query on associative and unordered associative containers. Use `find` when the iterator or mapped value is needed; use `count` when the number of equivalent keys is itself the answer.
...
The scale difference is asymptotic rather than tied to one tree shape: ordered associative member lookup is logarithmic in container size, while applying a linear search algorithm to its iterators is linear. The standard does not require a red-black tree or a fixed comparison count, so implementation folklore should not become the reason the card teaches.
