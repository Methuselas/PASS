...
- In C++20, prefer `std::erase_if`, which covers every standard container, or `std::erase`, which covers the sequence containers and strings. These free functions take the container, perform the complete removal, and return how many elements went; retain the erase-remove pairing for a sub-range or a container without such an overload.
