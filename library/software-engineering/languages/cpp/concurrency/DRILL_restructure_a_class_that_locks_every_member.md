...
## Instructions
1. Find every path where a public function reaches another public function on the same object — directly, through a stored callable, through a base-class pointer, or through a virtual dispatch that lands back on this object. Build one such nested acquisition with an ordinary mutex and again with a recursive one, run each under a watchdog, and record what happens.
...
5. Make one piece of the guarded state static and leave the rest per-object. Run threads on two objects against the static state while it is still guarded by the per-object mutex, and record the result. Decide what that does to the mutex, and say what it costs to go the other way and make the single mutex static for everything. Keep both mutexes for the rest of the exercise.
6. You now hold two mutexes with different lifetimes. Find every path that acquires both — any member function that touches per-object and static state in one call — and name the order they are taken in, or the single guard that takes both. Show that every such path does the same. If no path takes both, say so, and say what keeps that true as the class grows.
...
## Success Check
- The nested acquisition is run with both mutex kinds and the result recorded: the recursive mutex completes, with every inner acquisition counted as redundant work, and the ordinary mutex fails. That failure is undefined behaviour and need not be a hang — one mainstream implementation reports it by throwing `std::system_error` — so a run that expected a hang and saw an exception has found the same defect, not a fix.
...
- The per-object mutex over static state is run from two objects' threads and its count recorded; a count that comes out right is not evidence the guard is sufficient. The static state ends guarded by something whose lifetime matches it — a static mutex, an atomic, or a container that synchronises itself — and the per-object state is still on the per-object mutex. Both failures are stated: guarding static state per-object, which corrupts it, and collapsing everything onto one static mutex, which is safe and serializes every object against every other.
- Every path that acquires both mutexes takes them in one stated order, or acquires both through a single `std::scoped_lock`, and those paths are named individually; or no path acquires both, and what keeps that true is stated.
...
## Common Failures
- Switching to a recursive mutex to make the original version work, which converts a failure into repeated redundant work and hides where the critical section actually ends.
- Leaving a private helper that locks, so a public caller fails on the second acquisition or double-locks, depending on the mutex type.
