...
One part of the cost picture is stable across implementations and one is not. What holds everywhere measured so far is the gap at the bottom: guarding every access with a lock costs an order of magnitude or more over any mechanism that establishes safety once, because it pays on the path that dominates rather than on the path taken once. The call-once facility is also consistently more expensive than a function-local static, and more so on Windows than on Linux.

What does not transfer is the ordering among the cheap options. Whether a hand-rolled double-check beats, matches, or loses to a function-local static depends on the implementation's guard, the memory ordering, and the target's cost for an atomic load — on some targets the hand-rolled version is level with the static or slightly ahead. So do not choose between them on remembered ratios, and do not carry a number from one platform's benchmark as though it were a property of the mechanism. Measure on the target if the difference would matter.

That leaves the real reason to prefer the function-local static, which is the one that does transfer: it costs no more than the alternatives on any measurement, requires the least code, and carries none of the atomics-and-ordering reasoning burden. A mechanism that is free to get right is worth choosing even where a hand-written one might edge it out.
