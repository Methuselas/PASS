...
## Don't
- Don't build these out of a condition variable when one of them fits. They make the counter or phase contract explicit and remove hand-written predicate/lost-wakeup machinery. Whether an implementation uses locks or wins a benchmark is target-specific and not the reason to choose the abstraction.
...
Performance remains an empirical property of the standard-library implementation, operating system, contention pattern, and target. A ping-pong microbenchmark can compare wakeup paths on one build; it cannot establish that a semaphore is lock-free or universally cheaper. Choose the facility by coordination shape, then measure when its cost matters.
