...
- Establish the crossover with a benchmark rather than a rule of thumb, because it depends on three things at once: the sequence length, the cost per element, and the implementation. Of those three, expect the cost per element to move the answer furthest: a sequence short enough to look obviously not worth parallelizing can still pay when each element is expensive, while a long sequence whose body is one arithmetic operation loses outright because it is bound by memory traffic rather than by computation. Do not carry a length threshold from someone else's measurement as though it were a property of the algorithm - a threshold measured with one weight of per-element work says nothing about another.
