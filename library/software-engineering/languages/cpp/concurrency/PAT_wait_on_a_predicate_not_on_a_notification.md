...
- Don't use `notify_all` or periodic timeout polling to mask an incorrect state protocol. They may make a missed notification less visible while adding contention or latency; correctness must come from the mutex-protected predicate and notification rule. Choose one-versus-all from which waiters can make progress, and add a timeout only when timeout is part of the operation's real contract.
