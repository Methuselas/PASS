...
## Do
- Pick the member by what you are actually forbidding. Non-public constructors stop construction outright, and — as a side effect — stop derivation and containment too, since a class whose constructors are unreachable can be neither a base nor a member. Give such a constructor a body rather than defaulting it: measured on one implementation, a private defaulted constructor was bypassed when the class was brace-initialized as the base or member of an aggregate — the derived object built with empty braces compiled where the same object built without them was refused — while a private constructor with a body was refused both ways. A non-public destructor stops objects with automatic or static storage, because those require an implicit destruction the caller cannot perform, while leaving heap creation open; make it protected rather than private when derived classes must still exist. A non-public allocation function stops the ordinary new-expression for the class while leaving stack and static objects available, but it discourages heap objects rather than preventing them: the array form is looked up separately and has to be made non-public as well, a new-expression qualified with the global scope skips the class's allocation function by rule, and library facilities that allocate through an allocator — the shared-pointer factory, the standard containers — never call it. Against a class whose single-object allocation function was private, all four compiled.
...
- Don't try to determine whether an object is on the heap by comparing its address against a local variable's. The reasoning ignores that static objects live in a third region belonging to neither stack nor heap, so where the assumed layout holds they are misclassified; and the layout itself is not universal — measured on one 64-bit implementation the stack sat below both the heap and the static region, and the comparison reported every heap object as not on the heap. There is no portable way to answer the question at all, which makes redesigning so you never ask it the only reliable move.
...
Function-local statics carry an initialization property that is the point rather than a detail. They are constructed the first time control reaches them, so an unused one is never built, and their construction order relative to each other follows the order of use rather than the undefined cross-translation-unit ordering that afflicts statics at namespace scope. The cost is a check on each call to see whether construction has happened yet, for an object whose initialization or destruction does real work. One that is constant-initialized and trivially destructible is complete before the program runs, and the measured listing for such a function was a single address load with no check; giving the same object a destructor with a body brought the check back, because its destruction then has to be registered on first use.
