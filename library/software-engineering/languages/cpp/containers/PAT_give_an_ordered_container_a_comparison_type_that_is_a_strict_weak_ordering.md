...
## Do
- Supply a comparator *type* as the relevant template argument and a comparator object where construction requires one. A function name is a value rather than a type; use a function-pointer type when that representation is intentional, or prefer a function object or a C++20 lambda type that can inline and carry transparent overloads.
...
## Don't
- Don't supply a comparison that returns true for a value against itself. The operation requires a strict weak ordering; violating that precondition makes container and algorithm behavior invalid rather than guaranteeing any particular symptom.
...
The consequence is a violated algorithm/container precondition rather than a diagnostic. Once the relation is not a strict weak ordering, lookup, range queries, and ordering operations no longer have their specified foundation, so no particular duplicate, miss, or layout symptom should be taught as guaranteed.
