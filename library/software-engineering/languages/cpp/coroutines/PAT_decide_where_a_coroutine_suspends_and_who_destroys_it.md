...
- Keep in view what the handle can actually do, because it is a small interface: resume the coroutine, destroy it, ask whether it has reached its final suspension, and reach the promise object. Everything a client does goes through those, and the last is the channel every result and yielded value travels through, so reading one is always a question about whether the frame holding the promise still exists.
