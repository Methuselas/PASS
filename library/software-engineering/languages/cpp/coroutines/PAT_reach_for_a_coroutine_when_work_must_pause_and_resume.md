...
- Under C++23, take `std::generator` for the generator shape rather than writing a resumable type: it is a standard resumable type for a sequence yielded on request, so the promise type and the frame's ownership are already written. Tasks and asynchronous jobs still come from a library or your own type, and on the C++20 path so do generators.
...
The last of those goals explains why using coroutines in C++ feels like assembling rather than calling. The language supplies the suspension mechanism and almost no policy, so tasks and asynchronous jobs are things you or a library build on top, as generators were until C++23 supplied one — which is a deliberate trade of convenience for range.
