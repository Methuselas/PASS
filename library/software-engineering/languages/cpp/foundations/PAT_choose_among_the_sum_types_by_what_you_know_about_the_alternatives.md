...
**THEN** choose the vocabulary type by what the alternatives mean — `std::optional&lt;T&gt;` for value-or-absence, `std::variant` for a closed set, `std::expected&lt;T, E&gt;` as a C++23 option for value-or-typed-error, and `std::any` only for an intentionally open set
...
## Do
- Take meaning and compile-time knowledge as the axes. Absence without an explanation, failure with a typed explanation, a closed set of alternatives, and an open set are genuinely different contracts.
...
- Under C++23, prefer `std::expected&lt;T, E&gt;` when failure information is part of an ordinary return value. For a C++20 project, retain an established project result type or an explicit two-alternative `std::variant` design rather than silently raising the language floor.
...
- Don't generalize one retrieval rule across all these types. Dereferencing an empty `std::optional` is invalid while `.value()` reports failure; `std::get` on the wrong `std::variant` alternative throws and `std::get_if` returns null; value-form `std::any_cast` reports a mismatch while pointer-form returns null. Choose the access form deliberately.
...
- Does a failed operation need an error value, and if so is the C++23 `std::expected` option paired with a C++20 path?
