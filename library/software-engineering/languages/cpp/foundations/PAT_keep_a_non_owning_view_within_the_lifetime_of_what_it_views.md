...
- Don't store one as a data member without writing down what keeps its subject alive. Membership separates the moment of construction from the moment of use by an unbounded interval, which is exactly the gap the lifetime rule lives in, and nothing at the point of use suggests the question. That said, there are legitimate cases where storing a non-owning view as a member is appropriate and even necessary: when the object's lifetime is explicitly managed to be strictly contained within the lifetime of the data it views (for example, a per-frame entity that views data from a persistent resource manager), or when the view is immediately replaced on every update cycle (a transient handle that is rebuilt each frame). In such cases, document the lifetime relationship clearly, and ensure the owning object cannot be destroyed or replaced without the viewing object being destroyed or rebuilt first. The key is not to avoid stored views entirely, but to make the lifetime relationship explicit and enforceable.
