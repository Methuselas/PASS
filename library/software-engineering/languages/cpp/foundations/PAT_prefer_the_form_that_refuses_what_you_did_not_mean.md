...
- Use alias declarations rather than typedefs. Typedefs cannot be templatized, so the C++98 workaround is a nested type inside a struct, which callers must reach through a trailing `::type` and, inside templates, often prefix with `typename` because the compiler cannot tell whether the name is a type — C++20 dropped that requirement where only a type can appear, such as a member or alias declaration, and kept it elsewhere, such as a local variable in a function template. An alias template is a type directly and needs neither.
...
- Don't assume the old spelling is harmless because the code works today. `0` as a null pointer works until the call goes through a template; an unscoped enumerator works until a name collides or a comparison against an unrelated enumeration succeeds — it still compiles, now under a C++20 deprecation warning on a current compiler, and still means something nobody intended.
