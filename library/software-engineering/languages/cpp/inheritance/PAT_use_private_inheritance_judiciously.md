...
## Notes
Private inheritance means is-implemented-in-terms-of, the same as composition, so composition wins by default. Its narrow justification is redefining inherited virtuals or reaching protected members — the Widget/Timer onTick case. Even then a private nested class that publicly inherits the helper often works and adds the ability to stop derived classes overriding the virtual and to reduce compilation dependencies. One size argument once favored private inheritance and has weakened. An empty base can occupy no storage, and under the C++20 floor a composed member declared `[[no_unique_address]]` may do the same, so that optimization no longer forces inheritance. The attribute permits the saving rather than requiring it, and implementations differ: measured with MSVC in C++20 mode, a private empty base and a member using the vendor spelling `[[msvc::no_unique_address]]` each left the object the size of its one `int`, while the standard spelling was accepted with no warning at the highest level and ignored, doubling the object. Where the saving matters, choose the spelling per implementation and check `sizeof`.
