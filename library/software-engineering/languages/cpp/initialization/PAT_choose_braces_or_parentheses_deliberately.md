...
## Do
- Know what braces buy, since it is the case for making them the default. They are the only form usable everywhere — including default values for non-static data members, which parentheses cannot express, and objects whose only suitable constructor is explicit, which an equals sign cannot reach. An equals sign no longer rules out an uncopyable object in general: since C++17 copy-initialization through a non-explicit converting constructor involves no copy or move, and an atomic initialized with an equals sign compiled. They prohibit narrowing conversions among built-in types, so summing three doubles into an int is an error rather than a silent truncation. And they are immune to the most vexing parse: a declaration with empty parentheses declares a function, while one with empty braces default-constructs an object.
- Know what parentheses buy, since it is the case for the other default. They never divert a call to an initializer-list constructor. With `auto`, distinguish forms precisely instead of using this as a blanket reason: direct braces with one element deduce that element's type, while equals-plus-braces deduces an initializer list.
...
- Follow how far that preference goes, because it is further than "prefers." Braced copy and move are the exception: since a defect resolution applied to C++14, a braced initializer holding a single object of the same class initializes from that object, so a class with a list constructor and a conversion to the list's element type still copies and moves — measured, both went to the copy and move constructors, and material predating the resolution says otherwise. And the preference holds even when it produces an error: if the elements would require narrowing, the call is rejected rather than falling back on the other constructors. Only when no conversion to the element type exists at all does normal overload resolution resume.
...
The interaction with type deduction is worth stating precisely because older guidance predates its correction. In the C++20 baseline, `auto x{27}` deduces an integer, while `auto x = {27}` deduces an initializer list. Direct-list `auto` with more than one element is ill-formed. The delimiter and the presence of `=` are both part of the declaration's meaning.
