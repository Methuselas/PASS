...
- Introduce distinct Day, Month, and Year types three ways — with converting constructors, as aggregates, and with `explicit` constructors — and against each, compile both the raw transposed call and a braced call in the wrong order, recording which compile.
- Keep the form that rejects both. Constrain Month to its valid values, using predefined Month objects rather than a raw int or an enumeration of either kind. Try the scoped-enumeration version first, and record the point where it stops.
...
- The three wrapper forms are compiled against both calls and the results recorded: converting constructors let the raw transposed call through, aggregates refuse the raw call and accept the braced one in the wrong order, and only `explicit` constructors reject both. A run that tests only the raw call can settle on aggregates and still accept the transposition.
...
- Wrapping the values in aggregates, which refuse the raw call and accept a braced call in the wrong order.
