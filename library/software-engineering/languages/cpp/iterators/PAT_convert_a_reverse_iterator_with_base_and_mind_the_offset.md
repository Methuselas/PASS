...
- Prefer that spelling even where the other one compiles. Stepping back from the base fails to build wherever the iterators are plain pointers, since the result of a function call is not something you may modify. Mainstream standard libraries now give even the contiguous containers class-type iterators, so the other spelling usually compiles today — measured, it built on a vector and was refused on a reverse iterator over a raw array — which makes it a trap that waits for the code to meet a pointer.
- Keep the conversion lattice in view when planning any of this: a plain iterator converts implicitly to a constant one, a reverse one converts implicitly to a constant reverse one, and a plain iterator becomes a reverse one only by direct construction, because that constructor is explicit; nothing converts a constant iterator back to a mutable one.
...
- Don't reach for a cast to get a mutable iterator from a constant one. A cast by value is refused between the class types, but that does not mean no cast connects them: one standard library derives the mutable iterator from the constant one, so a reference cast compiled for both the vector and the list and wrote through the result with no diagnostic. Where the iterators are plain pointers a cast compiles as well. Each of those removes constness silently, which makes it a trap rather than a solution.
