...
- Say what a lambda captures even when nothing is captured. An empty capture clause is a statement that the closure captures nothing, and the compiler holds you to it for local variables that would need capturing. It does not certify that the closure depends on nothing outside itself: globals, statics, and local constants the body only reads as values remain reachable with no capture, and an empty-capture lambda reading all three compiled.
