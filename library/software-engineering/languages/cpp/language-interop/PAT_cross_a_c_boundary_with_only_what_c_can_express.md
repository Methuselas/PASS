...
- Restrict what crosses to what compiles in both languages: built-in types, pointers to objects, pointers to non-member or static functions, and structs whose definitions compile as C. Adding non-virtual member functions to such a struct will generally leave its layout alone, and a virtual function will not. A base class needs separating: one that declares data of its own moves the derived members and costs the struct its standard layout, while an empty base - a tag, a policy marker, a CRTP hook - costs neither, because the standard keeps such a class standard-layout and every ABI in practice gives the empty base no size. Measured on one implementation, the empty base left the size, the member offsets and the standard-layout answer identical to the plain struct, and the data-carrying base was what broke it. Check the case you have rather than ruling out bases as a category.
...
- Does every struct that crosses compile unchanged as C, with no virtual functions, and no base that declares data of its own?
