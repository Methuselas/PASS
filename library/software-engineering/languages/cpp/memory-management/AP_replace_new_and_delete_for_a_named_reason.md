...
6. **Pair every extra-parameter form with its matching release.** `PAT_pair_placement_new_with_placement_delete` owns this, and the cost of skipping it is silent at run time: the constructor throws, no matching release exists, and the allocation leaks. A compiler may warn at the declaration, but a near-match differing only by a qualifier draws the same warning and leaks just the same, so prove the pairing by running a throwing constructor rather than by a clean rebuild. Keep the ordinary release form alongside.
...
8. **Verify against the forms nobody wrote a test for.** Exercise zero-size direct calls where relevant, over-aligned storage, a derived size if inheritance is permitted, nothrow behavior, a constructor throwing after a placement form, an array of the class — which reaches the global array form unless the class declares its own — and every supported sized/aligned delete route.
