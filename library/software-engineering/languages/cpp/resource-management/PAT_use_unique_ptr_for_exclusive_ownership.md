...
- Return exclusive ownership even when some callers may want to share. A caller can explicitly move it into a shared pointer, which creates or adopts the necessary ownership state and may allocate a control block; the reverse conversion is not available, so the exclusive form leaves the caller a choice without prepaying for sharing.
...
- Don't keep the removed auto-pointer in C++20 code. It co-opted copying to mean transfer because the language had no move operations; modern exclusive ownership expresses transfer with an actual move and works with contemporary containers and algorithms.
