...
## Don't
- Don't assume that an index which is never negative should therefore be unsigned. The reasoning is sound about the values and incomplete about the code generated, because the compiler is permitted less with the unsigned form. What that is worth varies enormously and can be nothing: measured on a scalar comparison loop with a data-dependent branch, the signed, narrow-unsigned, and register-width forms came within three parts in a thousand of each other, while their generated code differed exactly as described. So take the signed default because it costs nothing and removes a trap, not because a figure is attached to it.
