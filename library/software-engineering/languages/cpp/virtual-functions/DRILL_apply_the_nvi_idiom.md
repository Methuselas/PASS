...
- Decide whether the wrapper may also hold a lock across the dispatch, write down which case you are in and the reason, and where it does hold one, enumerate every override that can run under it.
...
- The lock decision follows from where the overrides come from: the wrapper may hold a lock across the dispatch only where every override lives in this component and its locking is visible, and not where an override could be supplied by a caller. If it holds one, every override that can run under it is enumerated and each is inside this component. An unanswered question here is a deadlock that appears only once somebody else supplies an override.
