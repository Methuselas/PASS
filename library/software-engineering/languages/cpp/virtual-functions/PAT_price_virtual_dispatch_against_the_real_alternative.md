...
## Do
- Separate the two costs, because they differ by orders of magnitude. The dispatch itself is an extra pointer load and an indirect jump. Not being inlinable is the other cost, and it is the one that matters. Resist attaching a number to either: measured on one machine, the same virtual call was between two and three times an out-of-line direct call for a one-line callee and indistinguishable from it once the callee did forty iterations of work, so any single percentage is a statement about the callee's size rather than about dispatch.
...
- Measure the runtime alternatives rather than ranking them from first principles, because the ranking does not hold still. A hand-built table of function pointers indexed by a runtime value is what the compiler already generates for a virtual call, and measures like it - so that one is reliably a wash. A switch on a type tag is the one that surprises people in both directions: it has no vtable to load and no indirect target for the predictor to miss, and on a current compiler it beat virtual dispatch by a factor of two at three cases with the data access held equal. What it costs instead is maintenance and the correctness property the compiler was enforcing, which is the reason to refuse it even where it wins.
...
- Don't devirtualize by adding a type tag and a switch. It is more code, it may or may not be faster on the target you actually ship to, and it moves a correctness property the compiler was enforcing into a conditional somebody has to maintain - which is the objection that holds whichever way the measurement goes.
