---
object_id: PAT_portray_real_people_through_selective_behavioral_detail
object_type: pattern
name: Portray Real People through Selective Behavioral Detail
library_path:
  - writing
  - creative-nonfiction
  - foundations
stage_binding: 2 block
lane_fit: skill
foundation_role: specialization
routing_class: specialized
specialization_axis: genre
foundation_object_id: none
tags:
  - creative_nonfiction
  - character
  - real_people
  - behavioral_detail
  - caricature
cross_links:
  - rel: related_to
    target_object_id: PAT_preserve_factual_accountability_while_using_literary_craft
  - rel: related_to
    target_object_id: PAT_reveal_character_incrementally_through_behavior
  - rel: related_to
    target_object_id: PAT_build_round_characters_from_contradiction_and_desire
reference:
  source_title: "Creative Writing: Four Genres in Brief"
  author: David Starkey
confidence: high
references: []
variants:
  - variant_id: writing_creative_nonfiction_variant_render_fleeting_person_through_bounded_partiality
    variant_name: Render a Fleeting Person through Bounded Partiality
    variant_basis: constraint
    difference_from_foundation: Build a deliberately incomplete portrait from a bounded habitual impression, one active incident, the narrator's relation to the person, and an honest exit rather than implying a complete judgment.
    when_to_use: A real person appears briefly but must register as more than a label, function, or picturesque detail.
    when_not_to_use: The person is central enough to require evidence across several contexts, or the available material cannot support even the limited impression.
    absorbed_from_object_id: none
  - variant_id: writing_creative_nonfiction_variant_situate_person_through_culture_and_milieu
    variant_name: Situate a Person through Culture and Milieu
    variant_basis: context
    difference_from_foundation: Connect language, work, domestic practice, media, class, place, and historical pressure to observable conduct while keeping context distinct from unsupported causal explanation.
    when_to_use: Social and cultural conditions materially shape how readers understand the person's choices, constraints, relationships, or self-presentation.
    when_not_to_use: Context is being used to make the person representative of a group, to excuse unsupported motive claims, or to replace individualizing conduct.
    absorbed_from_object_id: none
---

# Portray Real People through Selective Behavioral Detail

## Pattern Rule
**IF** a creative nonfiction draft reduces an actual person to a general judgment, grievance, virtue, or collection of eccentricities
**THEN** select supportable actions, speech patterns, gestures, and contradictory qualities that let the person appear specific and three-dimensional without becoming a caricature
**ELSE** keep a narrow portrayal only when the person's limited presence performs one clear factual function and does not imply a complete character judgment

## Do
- Replace a label with an observed interaction that shows how the person moves, speaks, chooses, or responds under pressure.
- Choose intimate particulars that affect motive, relationship, or the reader's inference rather than details that merely make the subject look odd.
- Include evidence that complicates the narrator's strongest praise or resentment when the omitted side would materially falsify the person.
- Let a small physical or verbal habit recur only when its changing context reveals more than the habit itself.
- Scale the portrait to the essay: one charged gesture may distinguish a minor presence, while a central person needs evidence across more than one emotional register.

## Don't
- Treat the narrator's passion about a relative, former partner, adversary, or admired figure as sufficient proof of a one-sided characterization.
- Overemphasize grammar, accent, body, mannerism, or cultural difference until individuality becomes ridicule or stereotype.
- Invent a private motive to make observed behavior easier to explain.
- Accumulate colorful traits that never change the inquiry, relation, or action.
- Protect a favored person from every unflattering fact when the essay claims to examine them honestly.

## Checklist
- Readers encounter the person doing or saying something consequential rather than only receiving the narrator's verdict.
- Selected details distinguish this individual without turning difference into a joke or diagnosis.
- Positive and negative evidence appear in proportion to what the essay actually knows and needs.
- Inferred motives remain distinguishable from observable conduct.
- Removing a retained detail would weaken character, relation, or inquiry—not merely reduce color.

## Notes
Actual people cannot be redesigned to satisfy a cleaner character arc. The writer's craft lies in selecting behavior that makes a person legible while respecting the limits of observation and memory. Specificity increases vividness and ethical risk at the same time: the more memorable an idiosyncrasy becomes, the more carefully the writer must ask whether it reveals a human being or merely fixes them in the narrator's most convenient interpretation.

`writing_creative_nonfiction_variant_render_fleeting_person_through_bounded_partiality` gives a minor real person enough dimensionality to matter without pretending to know a whole life. Establish the narrator's angle of access, pair a bounded habitual impression with one active incident, and let the person leave the frame before the partial evidence hardens into a total judgment.

`writing_creative_nonfiction_variant_situate_person_through_culture_and_milieu` places conduct inside the social conditions that make it intelligible. Use specific language, labor, household practice, media, class, place, or historical pressure when it changes the available choices or relations, but distinguish what the context demonstrates from what the narrator merely infers about cause or identity.
